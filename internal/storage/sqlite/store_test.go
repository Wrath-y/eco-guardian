...
func TestV7ProjectUpgradesToChecksummedV8OnReopen(t *testing.T) {
	...
	if err = store.db.QueryRow(`SELECT count(*) FROM schema_migration_steps WHERE checksum IS NOT NULL`).Scan(&checksummed); err != nil || checksummed != currentSchemaVersion-1 {
