...
describe('simulation admission form', () => {
  ...
  it('opens an immutable historical run linked by the selected revision timeline', async () => {
    ...
    const fetchMock = vi.fn((url: string) => {
      ...
      if (url.endsWith(`/simulation-runs/${runID}`)) return Promise.resolve(new Response(JSON.stringify({ id: runID, revision_id: revisionID, input: { schema_version: 'v1', revision_id: revisionID, scene_id: 'single-target-30s', scene_version: 'v1', seed: 11, sample_count: 1000, metrics: [{ id: 'metric-dps', version: 'v1' }], config_hash: 'd'.repeat(64), budgets: {}, actions: [{ id: 'opening-strike', inputs: [{ id: 'amount', value: '10' }] }] }, input_hash: 'a'.repeat(64), result_hash: 'b'.repeat(64), fingerprint_hash: 'c'.repeat(64), reproducible: true, reasons: [], metrics: [{ id: 'metric-dps', version: 'v1', status: 'available', direction: 'higher_is_risk', value: '10', unit: 'points_per_second', confidence_low: '9', confidence_high: '11', sample_count: 1000, assumptions: ['independent_samples'] }], verification_refs: [] }), { status: 200 }))
      return Promise.resolve(new Response('', { status: 404 }))
    })
    vi.stubGlobal('fetch', fetchMock)
    const pinia = createPinia()
    ...
    expect(screen.getByText(/CI 9–11/)).toBeTruthy()
    expect(screen.getByText(/假设：independent_samples/)).toBeTruthy()
    ...
    const verify = screen.getByRole('button', { name: '用此 Run 发起复现验证' })
    await fireEvent.click(verify)
    expect(await screen.findByText(new RegExp(`已重建 ${runID}`))).toBeTruthy()
    expect(document.activeElement?.getAttribute('role')).toBe('alert')
  })
