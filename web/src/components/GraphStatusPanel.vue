...
<template>
  <section class="graph-status" :aria-label="`revision ${revisionID} 的图谱状态`">
    ...
    <template v-else-if="query.data.value">
      ...
      <template v-if="!compact"><p v-if="query.data.value.projection">projector {{ query.data.value.projection.projection_schema_version }}/{{ query.data.value.projection.projector_version }} · Nodes {{ query.data.value.projection.node_count }} · Edges {{ query.data.value.projection.edge_count }} · <code>{{ query.data.value.projection.graph_manifest_hash }}</code></p><p v-else>尚无可用投影摘要。</p><p v-if="query.data.value.job">Job {{ query.data.value.job.status }}<span v-if="query.data.value.job_phase"> · {{ query.data.value.job_phase }} {{ query.data.value.job_progress ?? 0 }}%</span> · <a :href="query.data.value.job.events_url">事件与进度</a></p><p v-if="query.data.value.provider">Graph/FTS/Vector：<span v-for="component in components" :key="component.name">{{ component.name }}={{ component.state }}；</span>观测于 {{ query.data.value.provider.observed_at }}</p></template>
